...
class Bee {
  ...
  private $framework = 'Sistema de Administración';
  ...
  private function init_load_config() {
    $file = 'bee_config.php';
    if(!is_file('app/config/'.$file)) { die(sprintf('El archivo %s no se encuentra, es requerido para que %s funcione.', $file, $this->framework)); }
  ...
  private function init_load_functions() {
    ...
    if(!is_file(FUNCTIONS.$file)) {  die(sprintf('El archivo %s no se encuentra, es requerido para que %s funcione.', $file, $this->framework)); }
  ...
  private function init_csrf() { $csrf = new Csrf(); }
  ...
  /** Método para ejecutar y cargar de forma automática el controlador solicitado por el usuario su método y pasar parametros a él. */
  private function dispatch() {
    ...
    else { $current_controller = DEFAULT_CONTROLLER; } // home Controler.php
  ...
  /* corremos nuestro archivo de phpthumb */
  /*public static function thumbsImages() {
    include_once(ASSETS.'plugins/thumb/Thumb.php');
    $thumb = new Thumb();
    return;
  }*/

  /* corremos el plugin de phpmailer */
 /* public static function funphpmil() {
